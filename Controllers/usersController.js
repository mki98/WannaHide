...
exports.getUser = tryCatch(async (req, res) => {
  ...
  const user = await Users.find({ username: req.params.username }).select(
    "publicKey -_id"
  );
  return res.status(200).json({ status: "Success", User: user });
});
exports.getContacts = tryCatch(async (req, res) => {
  let contacts = await Users.findById(req.user._id)
    .select(" friends -_id")
    .populate("friends");
...
exports.confirmReq = tryCatch(async (req, res) => {
  ...
  if (respond == true) {
    ...
    newChat = await Chats.create({
      users: [user._id, reqUser._id],
      shared: `${shared.astr}-${shared.bstr}`,
    });
    console.log("Created");
  }
  ...
  res
    .status(200)
    .json({ status: "Success", message: "Changes Saved", chat: newChat._id });
});
